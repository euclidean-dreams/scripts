...
from legacy.harmonic_transformer import HarmonicTransformer

DATA_FILE_NAME = "perpet-5_80.json"
INITIAL_FREQUENCY = 30
INITIAL_PEAK_SQUISH = 2
...
auto_correlator = HarmonicTransformer(signal, INITIAL_FREQUENCY, INITIAL_PEAK_SQUISH)
...
def peak_squish_slider_callback(attr, old, new):
    auto_correlator.update_peak_squish(new)
    data_source_manager.update()
...
harmonic_transsform_plot.line(
    source=data_source_manager.signal_derivative,
    line_color="#4f4cba",
    legend_label="Signal Derivative"
)
harmonic_transsform_plot.line(
    source=data_source_manager.signal_double_derivative,
    line_color="#b5b3ff",
    legend_label="Signal Second Derivative"
)
harmonic_transsform_plot.line(
    source=data_source_manager.comparison_signal,
    line_color="#ffbe0d",
...
harmonic_transsform_plot.line(
    source=data_source_manager.auto_correlated_signal_double_derivative,
    line_color="#8fe38f",
    legend_label="Output Signal double derivative"
)
harmonic_transsform_plot.legend.location = "top_left"
harmonic_transsform_plot.legend.click_policy = "hide"
...
harmonic_transform_integral_plot.dot(
    source=data_source_manager.auto_correlation_integral_data_double_derivative,
    color="#f2a2e1",
    size=100
)
